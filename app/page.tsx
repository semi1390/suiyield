...
import { ArrowRight, ChevronRight, Bell, Wallet, TrendingUp, Brain } from "lucide-react"
...
const STEPS = [
  {
    num: "01", icon: Wallet, title: "Connect your wallet",
    desc: "One click. Read-only connection to your Sui wallet. We never request transaction approvals, never touch your funds, and never store your private keys.",
    tag: "Non-custodial",
    mockup: (
  ...
  {
    num: "02", icon: TrendingUp, title: "See every opportunity",
    desc: "Every lending rate, DEX pool, and staking yield across Sui — ranked by APY, filterable by asset and risk level, updated every 5 minutes directly from the protocols.",
    tag: "113+ pools",
    mockup: (
  ...
  {
    num: "03", icon: Brain, title: "Let AI guide you",
    desc: "The AI Yield Advisor reads your exact on-chain positions, compares them to every live rate across the ecosystem, and tells you specifically where your capital should be right now.",
    ...
    mockup: (
      <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", background: "rgba(255,255,255,0.03)", borderRadius: 8 }}>
          <div style={{ width: 28, height: 28, borderRadius: "50%", background: "rgba(0,212,170,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: "#00D4AA", flexShrink: 0 }}>72</div>
  ...
  {
    num: "04", icon: Bell, title: "Get alerted instantly",
    desc: "Set a target APY for any asset or protocol. The moment a pool crosses your threshold, you get a Telegram message. Never miss a rate spike again.",
    tag: "Telegram alerts",
    mockup: (
      <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
        ...
        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", textAlign: "center" as const }}>Set once. Never miss a spike.</div>
      </div>
    ),
  },
]
...
export default function LandingPage() {
  ...
  const tickerItems = ["DEEP · 18.26%", "EEARN · 17.52%", "NS · 16.22%", "DEEP · 14.43%", "WAL · 12.48%", "USDC · 9.63%", "SUI · 6.11%", "USDT · 7.22%"]
    ...
    fetch("/api/yields").then(r => r.json()).then(d => {
      ...
      if (poolCount > 0) {
        setStats([
          ...
          { value: "8+", label: "Protocols" },
          { value: "4", label: "Position readers" },
          { value: totalTvl > 1e9 ? `$${(totalTvl/1e9).toFixed(1)}B+` : `$${(totalTvl/1e6).toFixed(0)}M+`, label: "TVL tracked" },
        ])
      }
    }).catch(() => {})

    fetch("/api/live-rates").then(r => r.json()).then(d => {
      const top = (d.data || [])
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0E1A", overflowX: "hidden", color: "#fff" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 24px 60px", textAlign: "center" }}>
        <div style={{ animation: "fadeUp 0.7s ease both" }}>
          ...
          <h1 style={{ fontSize: "clamp(36px, 7vw, 68px)", fontWeight: 800, color: "#fff", lineHeight: 1.05, marginBottom: 24, letterSpacing: "-0.03em" }}>
            Find the best yield<br />across <span style={{ color: "#4B8BFF" }}>Sui</span> —<br /><span style={{ color: "#00D4AA" }}>instantly.</span>
          </h1>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 24px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          {STEPS.map((step, i) => (
            <FadeIn key={i} delay={i * 0.1}>
              <div className="how-it-works-row" style={{ display: "grid", gridTemplateColumns: "80px 1fr", gap: 24, padding: "40px 0", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                <div>
                  <div style={{ fontSize: 48, fontWeight: 800, color: "rgba(0,212,170,0.15)", letterSpacing: "-0.04em", lineHeight: 1 }}>{step.num}</div>
                </div>
                <div className="step-inner" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, alignItems: "start" }}>
                  {/* Text */}
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(0,212,170,0.08)", border: "1px solid rgba(0,212,170,0.15)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  ...
                  <div className="step-mockup" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: 20 }}>
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.2)", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 12 }}>
                      {i === 0 ? "One click · Read-only · No approvals" : i === 1 ? "113+ pools · 8 protocols · Live rates" : i === 2 ? "AI-powered · Your wallet · Live data" : "Telegram · Any asset · Any threshold"}
                    </div>
                    {step.mockup}
                  </div>
                </div>
      ...
      <div style={{ background: "rgba(255,255,255,0.02)", borderTop: "1px solid rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "72px 24px" }}>
          <FadeIn>
            <div className="specs-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "start" }}>
              <div>
          ...
          <div className="footer-grid" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 40, marginBottom: 48 }}>
            ...
            <div>
              ...
              {[["Dashboard", "/app"], ["Explore", "/app/explore"], ["My Positions", "/app/positions"], ["Portfolio", "/app/portfolio"], ["Alerts", "/app/alerts"], ["Swap", "/app/swap"]].map(([label, href]) => (
                <Link key={href} href={href} style={{ display: "block", fontSize: 13, color: "rgba(255,255,255,0.35)", textDecoration: "none", marginBottom: 10 }}
            ...
            <div>
              ...
              {[["GitHub", "https://github.com/semi1390/suiyield"], ["Telegram Bot", "https://t.me/Suiyield_alerts_bot"], ["Sui Mainnet", "https://suiscan.xyz"]].map(([label, href]) => (
                <a key={href} href={href} target="_blank" rel="noopener noreferrer" style={{ display: "block", fontSize: 13, color: "rgba(255,255,255,0.35)", textDecoration: "none", marginBottom: 10 }}
                  onMouseEnter={e => (e.currentTarget.style.color = "#fff")}
                  onMouseLeave={e => (e.currentTarget.style.color = "rgba(255,255,255,0.35)")}>
                  {label} ↗
                </a>
  ...
  .how-it-works-row { grid-template-columns: 40px 1fr !important; gap: 12px !important; padding: 24px 0 !important; }
  .step-inner { grid-template-columns: 1fr !important; gap: 0 !important; }
  .step-mockup { display: none !important; }
